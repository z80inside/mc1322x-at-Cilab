...
int send_data(uint16_t panid, uint16_t src, uint16_t dst, uint8_t *pay)
{
	volatile packet_t *p;
	...
	if(p) {
		...
		compose_frame(p, &pinfo, pay, strlen(pay));
		tx_packet(p);				
		ret = 1;
	}
	return ret;
...
int send_ack(int cnt, uint8_t *pay)
{
	volatile packet_t *p;
	struct pkt_info pinfo;
	int ret = 0;
		...
		if(uart1_can_get()) {
			...
			case 'w':
				...
				*MACA_RXEND = end;
				...
				break;
			case 'd':
				send_data(0xaaaa, 0x2222, 0xFFFF, "hola");
				printf("autoack-tx --- ");
				break;
			case 'f':
				send_data(0xaaaa, 0x2222, 0x1111, "hola");
				printf("autoack-tx --- ");
				break;
			case 'g':
				send_data(0xaaaa, 0x2222, 0x1234, "hola");
				printf("autoack-tx --- ");
				break;
			case 'b':
				p = get_free_packet();
